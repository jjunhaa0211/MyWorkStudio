...
class GitDataProvider: ObservableObject {
    ...
    // Diff support
    @Published var diffResult: GitDiffResult?

    // Search
    @Published var searchQuery: String = ""

    // Conflict detection
    @Published var conflictFiles: [GitFileChange] = []

    // Error reporting
    @Published var lastError: String?

    // Pagination
    @Published var commitPage: Int = 0
    let commitsPerPage = 100
    ...
    func fetchFileDiff(projectPath: String? = nil, path filePath: String, staged: Bool, hash: String? = nil) -> String {
    ...
    func fetchParsedDiff(path filePath: String, staged: Bool, hash: String? = nil) {
    ...
    func stageFile(path filePath: String) {
        let safePath = GitDataParser.sanitizePath(filePath)
        let path = projectPath
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
    ...
    func unstageFile(path filePath: String) {
        let safePath = GitDataParser.sanitizePath(filePath)
        let path = projectPath
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
    ...
    func discardFile(path filePath: String) {
        let safePath = GitDataParser.sanitizePath(filePath)
        let path = projectPath
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
...
enum GitDataParser {
    ...
    static func parseConflicts(path: String) -> [GitFileChange] {
    ...
    static func parseDiff(filePath: String, rawDiff: String) -> GitDiffResult {
        // Check for binary
