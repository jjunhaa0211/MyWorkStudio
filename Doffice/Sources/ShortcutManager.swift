...
struct KeyShortcut: Codable, Equatable, Hashable {
    var keyEquivalent: String   // 예: "t", "k", "[", "1"
    var command: Bool           // ⌘
    var shift: Bool             // ⇧
    var option: Bool            // ⌥
    var control: Bool           // ⌃
...
let defaultShortcutMap: [ShortcutAction: KeyShortcut] = [
...
class ShortcutManager: ObservableObject {
    ...
    @Published var shortcuts: [ShortcutAction: KeyShortcut?] = [:] {
    ...
    func checkConflict(_ shortcut: KeyShortcut, excludingAction: ShortcutAction) -> ConflictResult {
        // 시스템 단축키 충돌 검사
