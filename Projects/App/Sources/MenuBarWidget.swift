    ...
    private var headerSection: some View {
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                ...
                Text(AppSettings.shared.companyName.isEmpty ? "AI Code Manager" : AppSettings.shared.companyName)
            ...
            HStack(spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 4) {
                        ...
                        if tab.provider == .codex || tab.selectedModel != .sonnet {
                            Text(tab.provider == .codex ? "CX" : String(tab.selectedModel.displayName.prefix(1)))
                                .font(Theme.mono(6, weight: .bold))
                                .foregroundColor(tab.provider == .codex ? Theme.orange : Theme.purple)
                                .padding(.horizontal, 3).padding(.vertical, 1)
                                .background((tab.provider == .codex ? Theme.orange : Theme.purple).opacity(0.1))
                                .cornerRadius(2)
                        }
                    }
                    HStack(spacing: 4) {
