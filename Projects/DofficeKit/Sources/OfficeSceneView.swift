...
public struct OfficeSceneView: View {
    ...
    private var selectedPluginFurnitureSource: PluginHost.LoadedFurniture? {
        guard let selectedFurniture,
              selectedFurniture.type == .plugin,
              let pluginFurnitureId = selectedFurniture.pluginFurnitureId else {
            return nil
        }
    ...
    private func characterActionPopover(tab: TerminalTab) -> some View {
                    ...
                    Menu {
                        ...
                        HStack(spacing: 6) {
                            Image(systemName: "person.text.rectangle")
                                .font(.system(size: 9))
                            Text(NSLocalizedString("office.action.role", comment: ""))
                                .font(Theme.mono(9, weight: .medium))
                            Spacer()
                            Text("\(char.jobRole.icon) \(char.jobRole.displayName)")
                                .font(Theme.mono(8))
                                .foregroundColor(Theme.textDim)
                        }
        ...
        .background(
            RoundedRectangle(cornerRadius: 12).fill(Theme.bgCard)
                .shadow(color: Theme.panelShadow.opacity(0.3), radius: 16, x: 0, y: 8)
        )
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            if let furniture = selectedFurniture {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if let pluginSource = selectedPluginFurnitureSource {
                        HStack(spacing: 5) {
                            Image(systemName: "puzzlepiece.extension.fill")
                                .font(.system(size: Theme.iconSize(7), weight: .bold))
                            Text(pluginSource.pluginName)
                                .font(Theme.mono(7, weight: .bold))
                                .lineLimit(1)
                        }
                        .foregroundColor(Theme.purple)
                        ...
                        .overlay(RoundedRectangle(cornerRadius: Theme.cornerMedium).stroke(Theme.purple.opacity(0.26), lineWidth: 1))
                    }
                    ...
                    if !pluginHost.officePresets.isEmpty {
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            ForEach(pluginHost.officePresets) { preset in
                                Button { applyPluginPreset(preset) } label: {
                                    HStack(spacing: 4) {
                                        VStack(alignment: .leading, spacing: 1) {
                                    ...
                                    .background(RoundedRectangle(cornerRadius: Theme.cornerMedium).fill(Theme.cyan.opacity(0.08)))
                                    .overlay(RoundedRectangle(cornerRadius: Theme.cornerMedium).stroke(Theme.cyan.opacity(0.2), lineWidth: 1))
                                }
                                .buttonStyle(.plain)
                            }
                        }
                                ...
                                ForEach(sortedPluginFurniture) { item in
                                    Button {
                                        ...
                                        VStack(spacing: 2) {
                                            ZStack {
                                                RoundedRectangle(cornerRadius: Theme.cornerMedium)
                                                    .fill(Color.white.opacity(settings.isDarkMode ? 0.08 : 0.96))
                                        ...
                                        .padding(.vertical, 3)
                                        .background(RoundedRectangle(cornerRadius: Theme.cornerMedium).fill(Theme.bgCard.opacity(0.6)))
                                        .overlay(RoundedRectangle(cornerRadius: Theme.cornerMedium).stroke(Theme.purple.opacity(0.15), lineWidth: 1))
                                    }
                                    .buttonStyle(.plain)
                                }
                            }
    ...
    private func placePluginFurniture(_ item: PluginHost.LoadedFurniture) {
        ...
        pluginPlacementNotice = "ADDED FROM \(item.pluginName.uppercased())"
        store.refreshLayout(with: manager.userVisibleTabs)
        store.saveCurrentLayout()
    }

    private func applyPluginPreset(_ preset: PluginHost.LoadedOfficePreset) {
        let inserted = pluginHost.applyOfficePreset(preset, to: map)
        guard !inserted.isEmpty else {
            pluginPlacementNotice = "NO OPEN SPACE FOR \(preset.pluginName.uppercased())"
            return
        }

        selectedFurnitureId = inserted.last?.id
        pluginPlacementNotice = "ADDED FROM \(preset.pluginName.uppercased())"
        store.refreshLayout(with: manager.userVisibleTabs)
        store.saveCurrentLayout()
    }
