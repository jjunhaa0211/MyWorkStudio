...
public struct OfficeLayoutSnapshot: Codable {
    public let cols: Int
    public let rows: Int
    public var tiles: [[TileType]]
    public var zones: [[OfficeZone?]]
    public var furniture: [FurniturePlacement]
    public var seats: [Seat]

    public init(cols: Int, rows: Int, tiles: [[TileType]], zones: [[OfficeZone?]], furniture: [FurniturePlacement], seats: [Seat]) {
        self.cols = cols; self.rows = rows; self.tiles = tiles; self.zones = zones
        self.furniture = furniture; self.seats = seats
    }
}

public final class OfficeLayoutStore {
    ...
    public func applyStoredLayout(to map: OfficeMap, preset: OfficePreset) {
        guard
    ...
    public func saveLayout(from map: OfficeMap, preset: OfficePreset) {
        guard let data = try? JSONEncoder().encode(map.layoutSnapshot()) else { return }
        defaults.set(data, forKey: key(for: preset))
    }

    public func resetSavedLayout(preset: OfficePreset) {
...
extension OfficeMap {
    public func layoutSnapshot() -> OfficeLayoutSnapshot {
    ...
    public func applyLayoutSnapshot(_ snapshot: OfficeLayoutSnapshot) {
    ...
    public func selectedFurniture(at coord: TileCoord) -> FurniturePlacement? {
        furniture
            .sorted { $0.zY < $1.zY }
            .reversed()
            .first { placement in
    ...
    public func movableAnchorId(for furnitureId: String) -> String {
    ...
    public func placeFurnitureGroup(anchorId: String, at newPosition: TileCoord) -> Bool {
...
extension OfficeMap {
    ...
    public func removeFurniture(id: String) -> Bool {
    ...
    /// 가구 추가 (위치 유효성 검증 포함)
    public func addFurniture(_ type: FurnitureType, at position: TileCoord, zone: OfficeZone, size: TileSize? = nil) -> FurniturePlacement? {
    ...
    public func addDeskSet(at position: TileCoord, zone: OfficeZone) -> Bool {
...
extension OfficeMap {
    ...
    public func moveVerticalWall(to newCol: Int) -> Bool {
    ...
    public func moveHorizontalWall(to newRow: Int) -> Bool {
...
public enum FurnitureEvent: String, CaseIterable {
    ...
    public static func events(for furnitureType: FurnitureType) -> [FurnitureEvent] {
...
extension OfficeMap {
    ...
    public func interactionTile(for furniture: FurniturePlacement) -> TileCoord? {
    ...
    public func availableInteractions() -> [(FurniturePlacement, FurnitureEvent)] {
