...
public struct AuditEntry: Codable, Identifiable {
    ...
    public init(tabId: String, projectName: String, eventType: AuditEventType, detail: String, isDangerous: Bool = false) {
        self.id = UUID()
        self.timestamp = Date()
        self.tabId = tabId
...
public class AuditLog: ObservableObject {
    ...
    public func log(_ type: AuditEventType, tabId: String, projectName: String, detail: String, isDangerous: Bool = false) {
    ...
    public func filteredEntries(types: Set<AuditEventType>? = nil, dangerousOnly: Bool = false) -> [AuditEntry] {
